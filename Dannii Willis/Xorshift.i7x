...
"Allow's Inform 7's random number generator to be replaced with one that will be consistent across all interpreters"
...
! Replace Inform's default random function with one that will use the xorshift prng (when enabled)
Replace random;
[ random range result mod temp;
	...
	! The xorshift prng (xorshift* for Glulx)
	.Begin;
	#ifdef TARGET_ZCODE;
...
! Using @random seed the xorshift prng
...
Inform allows you to seed the random number generator so that if you use the same seed the same sequence of 'random' numbers will be generated. Unfortunately it isn't consistent when you change interpreters or computers. This extension replaces Inform's built in random number generator with another one called xorshift, which will be consistent across all interpreters and computers. (Though not if you change from the Z-Machine to Glulx and vice versa.) Unlike the other extensions for randomness, this extension will affect every part of Inform that has random behaviour.

The xorshift generator is controlled by the "xorshift seed" variable. To turn it on, set the variable to any number other than 0. To turn it off, set it to 0. If you like, you can use the "seed the xorshift generator random" phrase, which sets the variable using a random number generator with the built in generator. The seed variable is continually updated as random numbers are generated, so if you want to reuse a seed, you should store it in another variable. Once you have set the seed variable no other code changes are needed.
