...
Without this extension, by default all players are in "verbose" mode by default and will see the full room description every time they visit a room.  At the user's option, they may request "brief" mode, which will print a description the first time and nothing on subsequent times, or "superbrief" mode, which will print nothing on the first time either.  In all cases, using "look" explicitly will show the full description regardless of the mode.
