Version 1/200510 of Title Page by Gavin Lambert begins here.
...
When play begins (this is the open the status window after title page rule):
	if the prior title page status is true:
...
Example: * Title page with splash image
...
Also at time of writing some additional steps are required after releasing before Quixe can load any image files, as it doesn't have access to the full blorb file.
	...
	*: "Splash"
	...
	The story author is "Gavin Lambert".  The story headline is "A Graphic Adventure".  Limbo is a room.
...
Example: * Customising the title page display
...
Example: *** Customising the title page menu
...
This example also demonstrates integration with Flexible Windows by Jon Ingold; when present, it will entirely hide the status window while the title page is shown and then show it again (or not, as appropriate) afterwards.  Menus by Dannii Willis is ok with this, but it's possible some other custom rules may want to do things involving the status window that aren't expecting it to be closed.  If this is a problem, then you can disable these rules with:
