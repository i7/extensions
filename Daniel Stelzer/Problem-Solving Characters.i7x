Version 2 of Problem-Solving Characters (for Glulx only) by Daniel Stelzer begins here.
...
"modified from the extension by Ron Newcomb"
...
An attempt can be untried, failed, partly successful, or successful (this is the status property).
...
An attempt has a number called the attempt index. [This is used to keep the attempts in chronological order.]
...
[	say "Recycling [A in agenda form].";	]
	...
	now A is available;
	now A is untried;
	now the attempt index of A is zero;
	repeat with C running through attempts fulfilled by A:
		now A does not fulfill C.

The attempt ticker is initially 1. [This is used to keep the attempts in chronological order. Zero means an unused attempt object or the goals of some NPC.]
...
To decide what attempt is a new attempt for (actor - a person) who'll try (suggestion - a stored action):
	let the circumvention be a random available attempt;
	now the attempt index of the circumvention is the attempt ticker; [This should keep everything in order.]
	increment the attempt ticker;
...
[	decide on the list of not in the past attempts which fulfill the act.]
	...
	sort the choices in attempt index order;
...
To decide which attempt is (act - a stored action) from the agenda of (performer - a person):
	...
	repeat with the item running through unavailable attempts:
...
To say (the act - an attempt) in agenda form:
	say "[act] ([if the act is hindered]hindered by the [meddling check rule of the act][else if the act is partly successful]solved despite the [meddling check rule of the act][else][status of the act][end if])".

To say sub-agenda for (act - an attempt) indenting (indentation level - a number) times:
	let L be the list of attempts which fulfill the act;
	sort L in attempt index order;
...
Example: * Baked Potato - Careful, it's hot.
	...
	*: "Baked Potato"
	...
	Include Problem-Solving Characters by Daniel Stelzer.
	...
	Test me with "agendas / Bob, take the potato / agendas / wait / agendas / wait / agendas / wait / agendas / wait / agendas / wait / agendas / wait".
...
We'll start with the hot potato example, then add the necessary code.
	...
	*: "Motive and Method"
	...
	Test me with "Bob, take the potato / agendas / wait / agendas / Bob, why did you attack the microwave / agendas / Bob, what are you doing / agendas / Bob, how will you take the potato / wait / agendas / Bob, why did you take the potato".
...
Now we'll add the code to handle the questions.  The first hurdle is the parser.  WHY and HOW questions take an entire command after them, but if we require a wording like WHY DID YOU TAKE THE POTATO, then we can capture the WHY/HOW in truth states that vary, cut the rest, and leave only TAKE THE POTATO for the parser.
	...
	*: Asking why, asking how are truth states that vary.
	...
	To decide whether questioning:
		if asking how is true, yes;
		if asking why is true, yes;
		no.
...
Finally, the money shot. The leftover command is parsed as taking the potato, but this Instead rule uses the truth states to determine that a question needs answering rather than a potato needing taking. We feed the current action to a phrase which searches the person's entire agenda for a match. If it finds taking the potato in Bob's past, present, or future, it then gives it to one of the phrases that gets the motive (why) or the method (how) for it. Note that we don't spend much time here dealing with differences in tense, endless WHYs or HOWs, creating attempt based rulebooks for a character, etc. which we would do in a real work. 
	...
	*: Instead of someone doing something when questioning:
		now the current interlocutor is the person asked;
