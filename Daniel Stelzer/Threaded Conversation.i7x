Threaded Conversation by Daniel Stelzer begins here.

"A conversation system tracking facts known, phrases spoken, and subjects of conversation."

"based on Chris Conley's version, based on Emily Short's version, based on [italic type]Alabaster[roman type]"

Include Plurality by Emily Short. 
Include Basic Screen Effects by Emily Short. 
Include Complex Listing by Emily Short.
Include Conversation Framework by Eric Eve.
Include Simple Followers by Daniel Stelzer. [Emily Short's version works too, for this, but I can't include both.]
	...
	repeat with item running through things which are mentioned by something:
		...
		[	 it's fair to assume that they're not secretly meant to be subjects.	]
			if the item is a subject:
				do nothing;
			otherwise:
				say "*** [The item] could be a subject.[line break]".
...
Rule for listing plausible quips (this is the standard quip plausibility rule):
	if the current interlocutor is conversation-hinted or the conversation hinting override flag is true:
		prepare a list of plausible quips;
	otherwise:
		prepare a list of plausible conversation-hinted quips;
	...
	otherwise:
		...
		if the number of filled rows in the Table of Scored Listing is positive, 
			say "[quip-suggestion-phrase][the prepared list delimited in disjunctive style].";
	now the conversation hinting override flag is false.
...
The conversation hinting override flag is a truth state that varies. The conversation hinting override flag is false.
A person can be conversation-hinted. A person is usually conversation-hinted. [A non-conversation-hinted person will only show conversation-hinted quips.]
A quip can be conversation-hinted. A quip is usually not conversation-hinted.
...
Understand "himself" as a person when the item described is male and the item described is the current interlocutor.
Understand "herself" as a person when the item described is female and the item described is the current interlocutor.
...
Check changing the subject (this is the default subject message rule):
	if the number of peripheral quips is 0:
		say "You can think of nothing to say at the moment." instead.
...
Book IV - Modified Leavetaking

[The say goodbye before going rule is not listed in any rulebook.]

This is the check interlocutor visibility rule:
	if the player cannot see the current interlocutor:
	[	say ">> INTERLOCUTOR RESET!";	]
		reset the interlocutor.

After saying hello to a person when the noun is not conversation-hinted:
	now the conversation hinting override flag is true. [Give hints for this one turn.]

Instead of saying hello to the current interlocutor (this is the greet current interlocutor rule):
	say "You have [the current interlocutor]'s attention.";
	now the conversation hinting override flag is true.

The can't greet current interlocutor rule is not listed in any rulebook.
The check interlocutor visibility rule is listed after the shadowing rule in the every turn rules.
