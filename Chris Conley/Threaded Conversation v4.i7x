Version 4/140528 of Threaded Conversation v4 by Chris Conley begins here.
...
[	A quip that indirectly follows another can be used any time after the first quip has been spoken.	]
...
[This rule is provided here as an unused stub, which could be hooked up to the THINK action if appropriate.]
...
A plausibility rule for a quip (called target) (this is the avoid topic-change where possible rule):
	...
	[In other words: we should not suggest to jump from the middle of one thread to the top of another thread, if we can avoid it.]
...
[	This is completely at the discretion of the author; if it's never used, nothing bad results.	]
...
Rule for avoiding talking heads (this is the default pause-construction rule):
	...
	[	But if instead the NPC's comment begins with quoted text, we do want to add a beat, to separate it from the quoted text that preceded.	]
...
Carry out starting a conversation with it about (this is the implicitly greet a potential new conversant rule):
	say "(implicitly greeting [the noun])[command clarification break]";
...
Threaded Conversation v4 ends here.
...
Character conversation queueuing and intentional conversation is also displayed, as well as some manipulation of physical objects through dialog.
	...
	*: "Blackened Blue"
	...
	first spiel is an NPC-directed beat-opened quip.
		the reply is "The Chief shuts the door and sits down. 'Look, McGorsky. You're a good cop but you got a bad break. I'm moving you to North Division until things calm down in Chinatown.'"
