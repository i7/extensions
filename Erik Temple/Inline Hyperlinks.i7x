...
To display (L - a list of text) using indices:
	let count be 1;
...
The last hyperlink that was set is recorded in two global text variables:
...
The player eventually recovers from the disorienting effects of the drug, and we track this using a variable called derangement. Because most of our movement occurs in situations that do not trigger the every turn and time advancement machinery, we hack the tracking of this variable into the routines where it is needed via the derangement abatement rule. 
	...
	*: 	"Systematic Derangement of the Inner Compass"
	...
	This is the derangement abatement rule:
		decrease derangement by 1;
		if derangement < 1:
			player recovers.
	
	Every turn when the player is deranged:
		follow the derangement abatement rule.
...
Since we are shortcutting normal processes with our new hyperlink processing rule, we also print a new command prompt, as well as follow the derangement abatement rule, to be sure that the player can eventually emerge from her unfortunate state.
