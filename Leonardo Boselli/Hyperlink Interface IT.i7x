...
Chapter - Responses
    ...
    Hyperlink Interface carry out listing exits rule response (B) is "Da [qui] [regarding the player][puoi] andare verso [list of viable directions].".
    Hyperlink Interface carry out thing listing rule response (A) is "Nelle vicinanze [regarding the player][if the number of visible listable things is 0]non [vedi] nulla d[']interessante[otherwise][vedi] [a list of visible listable things][end if].".
    ...
    Hyperlink Interface showing the hyperlink introduction text rule response (A) is "[line break]Leggendo '[story title]', vedrai nel testo un certo numero di collegamenti ipertestuali evidenziati. Clicca sui collegamenti per progredire nella storia. [if object hyperlink highlighting is true]Puoi cliccare un collegamento a un [o]oggetto[x] per esaminarlo. [end if][if direction hyperlink highlighting is true]Una [d]direzione[x] evidenziata indica che quella parola ti sposterà nella direzione specificata . [end if][if topic hyperlink highlighting is true]Un [t]argomento[x] di discussione evidenziato sposterà la conversazione verso quell[']argomento. [end if][paragraph break]Se [if the number of active hyperlink systems > 1]i collegamenti ipertestuali nelle righe qui sopra[otherwise]il collegamento ipertestuale nella riga qui sopra[end if] non spicca tra il testo che lo circonda, scrivi [o]HYPERLINKS[x] per modificarne lo stile.[run paragraph on]".
