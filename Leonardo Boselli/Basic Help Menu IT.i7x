Basic Help Menu IT by Leonardo Boselli begins here.
...
"Introduzione a '[story title]'"	a table-name	"[paragraph break][story description]"	a rule
"Istruzioni per giocare"	Table of Instruction Options	--	--

Table of Instruction Options
title	subtable	description	toggle
"Cos[']è l'Interactive Fiction?"	a table-name	"[paragraph break]Questo gioco è un[']opera di Interactive Fiction (IF). Nell[']IF interpreti il personaggio principale di una storia. Scrivi comandi che determinano le azioni del personaggio ed il flusso della sceneggiatura. Alcuni giochi di IF includono immagini e suoni, ma non tutti: è la tua immaginazione che li fornisce. D[']altra parte, è disponibile un[']ampia gamma di azioni: mentre in altri giochi sei limitato a sparare, muoverti o cercare oggetti con un click del mouse, IF permette molte altre possibilità."	a rule
"Che cosa fare di '[command prompt]'?"	a table-name	"[paragraph break]Il segno '[command prompt]' rappresenta la domanda del gioco: 'Bene, che cosa vuoi fare ora?' Puoi rispondere scrivendo un[']azione -- solitamente un verbo all[']imperativo, a volte seguito da preposizioni ed oggetti. Così, per esempio, GUARDA, GUARDA LA SPADA, PRENDI LA SPADA."	a rule
...
NOTA: Questo è una lieve modifica dell'estensione originale Basic Help Menu di Emily Short. Ho aggiunto solo il comando AIUTO e tradotto i testi.
