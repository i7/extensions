Version 1/140607 of Distantly Visible Things by Marc Von Der Heiden begins here.

"Provides flexible handling of things that can be seen in multiple rooms, but - unlike backdrops - still have one distinct location. Version for Inform build 6G60, can also be used with German language extension by Team GerX."
...
Chapter 5a - English Standard refusal message for distantly visible things (for use without German by Team GerX)
...
Rule for printing a refusal for distantly handling a thing (called target) (this is the standard printing a refusal for distantly handling rule):
	say "You need to get closer to [the target][unless best route from the location of the player to the location of the target is nothing], which is to the [best route from the location of the player to the location of the target][end unless].".

Chapter 5b - German Standard refusal message for distantly visible things (for use with German by Team GerX)

Rule for printing a refusal for distantly handling a thing (called target) (this is the standard printing a refusal for distantly handling rule):
	say "Du solltest näher an [den target] herangehen[unless best route from the location of the player to the location of the target is nothing], [*der* target] sich von hier aus im [best route from the location of the player to the location of the target] befind[et target][end unless].".
...
This version of the extension is written for Inform build 6G60. It doesn't provide the adaptive prose feature of Inform build 6L02. When used with the German language extension by Team GerX, the standard message is translated accordingly. 
There is - however - a newer version for Inform build 6L02 available.
...
If we wanted to override the behavior in its entirety, we could replace the "for" rule that comes with the extension (called the standard printing a refusal for distantly handling rule). 
